...
Manifest input supports the existing `projects`, `project_dirs`, `lectures`, and `lecture_projects` shapes plus benchmark/paper `suites` entries. Paper bundle manifests that point to a nested `paper_bundle.matrix_manifest` are resolved to the nested suites while keeping report outputs public-safe.
